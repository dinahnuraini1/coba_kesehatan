...
if (isset($_GET['id'])) {
    ...
    $sql_select = "SELECT gambar FROM tb_gambar WHERE id = '$gambar_id'";
    ...
    if ($result->num_rows > 0) {
        ...
        $sql_delete = "DELETE FROM tb_gambar WHERE id = '$gambar_id'";

        if ($conn->query($sql_delete) === TRUE) {
            // Hapus file gambar dari penyimpanan (jika perlu)
            $file_path = "gambar_gallery/" . $gambar_file;
            ...
            echo "<script>window.location.href = 'index.php#gallery';</script>";
            exit;
        } else {
