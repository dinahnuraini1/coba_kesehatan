...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "SELECT id, nm_user, password FROM user WHERE nm_user = ?";
    ...
    if ($stmt = mysqli_prepare($conn, $sql)) {
        ...
        if (mysqli_stmt_execute($stmt)) {
            ...
            mysqli_stmt_bind_result($stmt, $id, $nm_user_db, $password);
            ...
            if (mysqli_stmt_num_rows($stmt) == 1) {
                ...
                // Tampilkan pesan kesalahan jika username tidak ditemukan
                echo "<script>alert('Username atau password salah !!')</script>";
                echo "<script>window.location.href = 'login.php';</script>";
                exit;
            }
        } else {
            // Tampilkan pesan kesalahan umum jika gagal mengeksekusi statement
        ...
        // Tampilkan pesan kesalahan jika gagal menyiapkan statement
        echo "<script>alert('Oops! Terjadi kesalahan. Silakan coba lagi nanti.')</script>";
        echo "<script>window.location.href = 'login.php';</script>";
        exit;
    }
